...
const StudentPage = async () => {
  const dbStudents = await prisma.student.findMany({
    orderBy: [{ class: "asc" }, { gender: "desc" }, { student_id: "asc" }],
  });
  ...
  return (
    <div className="w-full h-full flex flex-col">
      <header className="py-5 bg-white border-b border-[#e1e7ef] flex flex-col sm:flex-row items-center justify-center sm:justify-between px-6 lg:px-8 w-full">
        <div>
          ...
          <p className="text-center sm:text-start mt-5 sm:mt-0 text-sm font-[Prompt] text-gray-500">
            จัดการ/เพิ่ม/ลบ รายชื่อนักเรียน S.M.T. ในระบบ
          </p>
        </div>
      </header>
      <Suspense
